...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        try {
            ...
            // 3. Generate Content
            const prompt = `
このスマートフォンの画面収録動画を分析してください。
ユーザーが意味のある操作（タップ、クリック、入力、スクロール停止など）を行った重要なステップを特定してください。
...
1. **action (操作)**: 「〜します」の形式で統一すること（常体・丁寧語）。
   - 良い例: 「設定ボタンをタップします」「テキストを入力します」「画面を下にスクロールします」
   - 悪い例: 「設定ボタンをタップしてください」「テキストを入力する」「〜をタップした」
...
## 重要な注意点
- **タイムスタンプの選定**: 操作対象のボタンやテキストが**画面にはっきりと映っている瞬間**（操作の直前〜操作中）の時間を正確に特定してください。画面遷移中や、要素が隠れた後の時間は選ばないでください。
- **視覚的根拠**: 画像と説明が一致するように、画面上に実際に表示されているテキストやアイコンに基づいて説明を作成してください。
