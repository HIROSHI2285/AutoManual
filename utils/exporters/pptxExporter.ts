...
/**
 * 画像のサイズ（幅・高さ）をBase64から取得するユーティリティ
 */
function getImageDimensions(base64: string): Promise<{ width: number; height: number }> {
    return new Promise((resolve) => {
        ...
        img.onerror = () => {
            resolve({ width: 0, height: 0 }); // 失敗時は0
        };
        img.src = base64;
    });
}
...
/**
 * ナンバリング用SVGロゴ生成（完全中央固定版）
 * 座標を64に、dyで微調整を加え、PPT上での垂直方向のズレを解消しました
 */
function createStepNumberSvg(number: number): string {
    ...
    <svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 ${size} ${size}">
        ...
        <text x="64" y="64" dy=".1em" text-anchor="middle" dominant-baseline="central" alignment-baseline="middle" fill="white" font-family="Arial, sans-serif" font-weight="bold" font-size="70px">${number}</text>
    </svg>`;
...
async function addStepToSlide(slide: any, pptx: any, step: any, xPos: number, isTwoCol: boolean) {
    ...
    // 4. 画像 (縦横判定と調整)
    if (step.screenshot) {
        const { width, height } = await getImageDimensions(step.screenshot);
        const isLandscape = width > height;

        let imgWidth = isTwoCol ? 4.8 : 8.5;
        let imgHeight = isTwoCol ? 3.3 : 4.0;

        // 2カラム時の画像位置をご指示通り下げて調整
        let imgY = isTwoCol ? 3.0 : 2.6;

        slide.addImage({
            data: step.screenshot,
            x: isTwoCol ? xPos + 0.05 : (11.69 - imgWidth) / 2,
            y: imgY,
