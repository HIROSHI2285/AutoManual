...
/**
 * 紺色の円形ナンバリング（中心ズレを防ぎつつ、削れ防止用に描画領域を拡大）
 */
function createStepNumberSvg(number: number): string {
  // SVGのキャンバスサイズを大きくし、円の周囲に余白を持たせることで見切れを防止
  const size = 56;
  const radius = 20;
  const svg = `
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* コンテナを十分確保し、overflow:visibleで削れを防止 */
    .num-icon-wrapper { 
    ...
    .detail-text { margin-left: 18mm; font-size: 10.5pt; margin-bottom: 5mm; white-space: pre-wrap; color: #000; }
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    if (i > 1) {
      ...
      if (titleImageData) {
        ...
        // ヘッダータイトルの表示高さを8mmに拡大
        const h = 8;
        const imgWidth = (imgProps.width * h) / imgProps.height;
        pdf.addImage(titleImageData, 'PNG', 15, 6, imgWidth, h);
      }
