...
**【重要：抽出粒度の最適化（少し詳しめの細かさ）】**
- 作業者の**「意図的かつ明確なアクション」はすべて独立したステップとして抽出**してください。（例：「設定を開く」「部品Aを外す」「調味料を入れる」などの意味のある動作ごとに必ず分ける）
- マニュアルを見る人が迷わないよう、**操作の区切りごとに少し手厚く（細かめに）画像を抽出**することを推奨します。
- ただし、以下の**完全に意味のない微細な動作や待機時間は除外**してください：
  - PC操作の場合: 単なるマウスの移動、ホバー状態、入力のないフォーカス移行
  - 実作業の場合: 意味のないカメラの揺れ、単なる確認のみで動作が伴わない時間
  - 目的のない短い移動や迷い動作
  - ローディング中、アニメーション、または次の作業までの明らかな待機プロセス
- これにより、初心者が読んでも手順が飛ばず分かりやすい、**丁寧で少し細かめのステップ数**を抽出します。
