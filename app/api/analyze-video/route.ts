...
## タイムスタンプ選定ルール（最重要 — これを守らないと画像と説明文が致命的にズレます）

1. **操作の結果が画面に完全に反映され、画面が安定した瞬間のタイムスタンプを選んでください。**
   - アニメーション・遷移・読み込みが完全に終わった後の、静止した画面を選ぶこと。
   - 具体例：
     - 「設定メニューを開きます」→ メニューが**完全に展開されて静止した**フレーム。
     - 「完了ボタンをクリックします」→ 次の画面・ダイアログが**完全に表示された**フレーム。
     - 「ファイルを保存します」→ 保存完了の確認が**表示された**フレーム。
2. **タイムスタンプは操作完了の2〜3秒後を目安に選んでください。** 操作直後ではなく、結果が安定して表示されている瞬間です。
3. **絶対に禁止する選定パターン：**
   - ❌ 操作を実行する**直前**のフレーム（ボタンを押す前の画面など）
   - ❌ アニメーション・遷移の**途中**のフレーム
   - ❌ 読み込みスピナー・プログレスバーが表示されている最中
   - ❌ マウスカーソルがボタン上にあるがまだクリックしていない状態
4. **actionの内容と映像が一致すること**: actionに書いた操作の**結果**が、そのフレームに映っていなければなりません。
5. **【必須】各ステップのtimestampは必ず異なる値にすること。同じ秒数を2つ以上のステップに使用することを禁止します。**
6. **動画全体の時間軸に沿って均等に分散させること。** 動画の前半だけに集中したり、特定の数秒間に複数のステップを詰め込んではいけません。
7. ステップ間のtimestampは**最低5秒以上の間隔**を空けてください。
