        ...
        <div className={`manual min-h-screen transition-all duration-700 ease-in-out ${isEditMode ? 'bg-[#f8fafc] pl-[80px] max-w-none' : 'bg-white'}`}>
            ...
            <div className={`manual__header sticky top-0 z-[90] py-6 -mx-8 px-8 border-b transition-all duration-500 border-transparent font-noto ${isEditMode
                ...
                <div className={`mx-auto flex items-center justify-between ${isEditMode ? 'max-w-none px-4' : 'max-w-7xl'}`}>
                            ...
                            <>
                                ...
                                <div className="flex items-center gap-3 mb-4 animate-fade-in">
                                    ...
                                    <button
                                        ...
                                        className="h-10 px-6 rounded-lg text-slate-950 font-black text-xs uppercase tracking-widest hover:bg-slate-100 transition-all active:scale-95 whitespace-nowrap"
                                    >
                                        キャンセル
                                    </button>
                                    <button
                                        ...
                                        className="h-10 px-8 rounded-xl bg-purple-600 text-white font-black text-xs uppercase tracking-widest shadow-xl shadow-purple-200 hover:bg-purple-700 transition-all active:scale-95 border border-white/10 whitespace-nowrap"
                                    >
                                        変更を保存
                                    </button>
                                    <button
                                        onClick={handleSaveAndExit}
                                        className="h-10 px-10 rounded-xl bg-slate-950 text-white font-black text-xs uppercase tracking-widest shadow-2xl hover:bg-black transition-all transform hover:-translate-y-0.5 active:scale-95 border border-white/20 whitespace-nowrap"
                                    >
                                        保存して終了
                                    </button>
                                </div>
                                <input
                                    ...
                                    className="manual__title text-5xl font-black text-slate-950 tracking-tighter leading-tight bg-transparent border-b-2 border-purple-200 focus:border-purple-600 focus:outline-none transition-colors w-full placeholder-slate-300 mb-3"
                                    placeholder="マニュアルのタイトル"
                                />
                                <textarea
                                    value={manual.overview}
                                    onChange={(e) => onUpdateManual && onUpdateManual({ ...manual, overview: e.target.value })}
                                    className="manual__overview text-slate-800 font-bold text-lg w-full leading-relaxed bg-transparent border border-purple-200 rounded-lg p-3 focus:border-purple-600 focus:outline-none transition-colors min-h-[80px] resize-y placeholder-slate-300"
                    ...
                    <div className={`manual__actions flex items-center gap-3 shrink-0 ml-4 ${isEditMode ? 'hidden' : ''}`}>
                        {!isEditMode ? (
                            <>
                                {/* View Toggle */}
