...
## タイムスタンプ選定ルール（厳守）
以下のルールに従ってください。これを守らないと、画像と説明文が致命的に一致しなくなります。

1. **操作の結果が画面に反映された直後**の、画面が完全に静止しているタイムスタンプを選んでください。
   - 例：「設定メニューを開きます」→ メニューが**開いた状態**のフレームを選ぶ。
   - 例：「完了ボタンをクリックします」→ 完了ダイアログが**表示された状態**のフレームを選ぶ。
   - 例：「ネジを締めます」→ ネジが**締まった後**の状態のフレームを選ぶ。
2. **actionの内容と映像が一致すること**: actionに書いた操作対象・操作結果が、そのタイムスタンプのフレームに映っていなければなりません。
3. 画面遷移アニメーションの途中や、読み込み中のスピナーなどは絶対に避けてください。
4. **【最重要】各ステップのtimestampは必ず異なる値にすること。同じ秒数を2つ以上のステップに使用することを禁止します。**
5. **動画全体の時間軸に沿って均等に分散させること。** 動画の前半だけに集中したり、特定の数秒間に複数のステップを詰め込んではいけません。
6. ステップ間のtimestampは**最低3秒以上の間隔**を空けてください。
