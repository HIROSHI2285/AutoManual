...
**【重要：抽出粒度の最適化（中程度の細かさ）】**
- ユーザーの**「意図的かつ明確なアクション」はすべて独立したステップとして抽出**してください。（例：Aボタンをクリック、続いてBフィールドにテキスト入力、続いてCボタンをクリックする場合は、無理に1つにまとめず3つのステップに分ける）
- ただし、以下の**意味のない微細な操作は除外**してください：
  - 単なるマウスの移動やホバー（カーソルを合わせただけ）状態
  - クリックしただけで状態が変わらない入力フォームへのフォーカス
  - 目的のない短いスクロールや迷い動作
  - ローディング中やアニメーションなどの待機プロセス
- これにより、「細かすぎて冗長」でもなく「まとめすぎて手順が飛ぶ」こともない、**分かりやすい中程度のステップ数**を抽出します。（動画の長さや操作の複雑さに応じて、必要なステップ数は適切に可変させてください。）
