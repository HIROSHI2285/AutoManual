...
function createStepNumberSvg(number: number): string {
  // サイズを64に拡大。円の半径(16)に対してキャンバスを大きく取ることで、
  // ページ後半で発生するレンダリングのズレを物理的に吸収します。
  const size = 64;
  const radius = 16;
  const svg = `
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .cover-page {
        ...
        border-bottom: 5mm solid #1e1b4b; /* 下部ラインを追加 */
    }
...
export async function generateAndDownloadPdf(manual: ManualData, layout: 'single' | 'two-column' = 'single', safeTitle: string): Promise<void> {
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    if (i > 1) {
      ...
      // ページ番号 (色を紺色に変更、サイズを9ptに微調整)
      pdf.setFontSize(9);
      pdf.setTextColor(30, 27, 75); // 紺色 (#1e1b4b)
