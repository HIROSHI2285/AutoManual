 ...
 * ナンバリング用SVGロゴ生成
 * PDF版の成功ロジックを完全トレース。
 * 数字のフォントをArial Blackにすることで、幾何学的な中央配置を確実にします。
 */
function createStepNumberSvg(number: number): string {
    ...
    <svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 ${size} ${size}">
        ...
        <text x="50%" y="50%" dominant-baseline="central" text-anchor="middle" fill="white" font-family="Arial Black, Arial, sans-serif" font-weight="900" font-size="75px">${number}</text>
    </svg>`;
...
function addHeaderFooter(slide: any, pptx: any, title: string, pageNum: number) {
    ...
    // フッターライン (ご要望通り位置を 7.8 へ)
    slide.addShape(pptx.ShapeType.line, { x: 0.8, y: 7.8, w: 10.1, h: 0, line: { color: NAVY, width: 0.6 } });
    // ページ番号 (ラインとの間隔を空けるため 8.0)
    slide.addText(pageNum.toString(), { x: 10.0, y: 8.0, w: 0.9, h: 0.2, fontSize: 12, color: NAVY, fontFace: FONT_FACE, align: 'right' });
}
...
function addStepToSlide(slide: any, pptx: any, step: any, xPos: number, isTwoCol: boolean) {
    ...
    if (step.screenshot) {
        ...
        const imgY = isTwoCol ? 2.8 : 2.9; // OKをいただいた位置を維持
