...
### 3. AI・APIチューニングと最適化
- **プロンプト進化**: 画面の変更やポップアップを積極的に捉え、網羅性を重視するプロンプトへの粒度調整を実施。
- **モデルの統一・高速化**: 座標検出とマニュアルテキスト生成の一部で使われていた旧モデル(`gemini-2.0-flash`)を、メイン解析と同等の最新主力モデルである `gemini-2.5-flash` へ引き上げ、システム全体の精度と速度を底上げ。
...
- **縦2行（A4縦）レイアウト実装**: `two-row-vertical` オプションを新設。
  ...
  - **PDF**: A4縦レイアウトに最適化し、画像表示エリアを85mmまで拡大した美しい2ステップ分割レイアウトを確立。
- **エクスポートの混在使用対応**: PDF、Word、PPTXのすべてにおいて、出力ループ中でそのステップごとの `layout` を判定し、1個のファイル内で1列・2列・縦2行レイアウトをシームレスに混在・改ページして出力できるように再実装完了。
