...
export async function generateAndDownloadPdf(manual: ManualData, layout: 'single' | 'two-column' = 'single', safeTitle: string): Promise<void> {
  ...
  const headerImageData = createTextAsImage(manual.title, 32, '#1e1b4b');
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    if (i > 1) {
      ...
      if (headerImageData) {
        ...
        const w = (props.width * 6) / props.height;
        pdf.addImage(headerImageData, 'PNG', 15, 7, w, 6);
      }
