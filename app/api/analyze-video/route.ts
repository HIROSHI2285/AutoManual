...
## タイムスタンプ選定ルール（最重要 — 画像と説明文の整合性を決定します）

**基本方針: 「操作する直前」の画面を選んでください。**
マニュアルの読者が「今、自分の画面がこう見えているはずだ」と確認できる画像が最も有用です。

1. **各ステップのtimestampは、その操作を実行する「直前」の、操作対象が画面に表示されている瞬間を選んでください。**
   - 具体例：
     - 「設定メニューを開きます」→ 設定メニューの**ボタンが画面に見えている**フレーム（クリックする前）。
     - 「完了ボタンをクリックします」→ 完了ボタンが**画面に表示されている**フレーム（押す前）。
     - 「ファイル名を入力します」→ 入力欄が**空の状態で表示されている**フレーム。
     - 「ネジを締めます」→ ネジと工具が**準備された状態**のフレーム。
   - これにより、読者は「この画面で、ここをクリック/操作する」と直感的に理解できます。

2. **最後のステップだけは例外: 作業完了後の結果画面を選んでください。**
   - 例：「保存が完了しました」→ 保存完了のメッセージが表示された画面。
   - これにより、読者は作業の最終結果を確認できます。

3. **次のステップの画像が前のステップの「結果確認」になるように、タイムスタンプの順序を意識してください。**
   - ステップ1で「設定を開く」→ ステップ2の画像は「設定が開いた後の画面」になる → 自然に結果確認ができる。

4. **絶対に禁止する選定パターン：**
   - ❌ アニメーション・遷移の途中のフレーム
   - ❌ 読み込みスピナー・プログレスバーが表示されている最中のフレーム
   - ❌ ぼやけた画面やフォーカスが合っていないフレーム

5. **【必須】各ステップのtimestampは必ず異なる値にすること。**
6. **動画全体の時間軸に沿って均等に分散させること。**
7. ステップ間のtimestampは**最低3秒以上の間隔**を空けてください。

box_2d は 0-1000 の範囲に正規化してください（操作対象のバウンディングボックス — 操作する直前の位置）。
`;
